...
def run_ewc_experiment():
    """
    Runs the EWC (Elastic Weight Consolidation) experiment using ResNet50 model on multiple datasets.

    This function collects datasets from different paths, initializes the model, and performs the EWC experiment
    for different combinations of orders and lambda values.

    Logs results to folder <ewc_logs_path>/RESNET_FRIDAY_2/<dataset_order>/<lambda>.
