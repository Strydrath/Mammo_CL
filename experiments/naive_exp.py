...
for i in range(0, len(orders)):
        ...
        name_of_experiment = "RESNET/0.0001"+names[i]
        experiment = exp(train1, train2, train3, test1, test2, test3, val1, val2, val3, orders[i], name_of_experiment)
        naive_experiment(model, experiment.train_set, experiment.test_set, experiment.val_set , device, name_of_experiment, 0, epochs=10, lr=0.0001, batch_size=32)
...
for i in range(0, len(orders)):
        ...
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        model = ResNet50(num_classes)
        name_of_experiment = "RESNET/0.001"+names[i]
        experiment = exp(train1, train2, train3, test1, test2, test3, val1, val2, val3, orders[i], name_of_experiment)
        naive_experiment(model, experiment.train_set, experiment.test_set, experiment.val_set , device, name_of_experiment, 0, epochs=10, lr=0.0001, batch_size=32)
